...
As parsidan grows and the authentication method changes, only these tests
should be updated.
...
class TestAuthentication(TestController):
    ...
    """
    test_user = 'webmaster@parsidan.com'
    test_pass = 'webmaster'
    ...
    def test_forced_login(self):
        ...
        resp = self.app.get('/contribution/', status=302)
        ok_( resp.location.startswith('http://localhost/login'))
        # Getting the login form:
        resp = resp.follow(status=302)
        ok_( resp.location.startswith('http://localhost/authentication/login'))
        resp = resp.follow(status=200)
        form = resp.form
        # Submitting the login form:
        form['login'] = self.test_user
        form['password'] = self.test_pass
        post_login = form.submit(status=302)
        # Being redirected to the initially requested page:
        ok_(post_login.location.startswith('http://localhost/authentication/post_login'))
        initial_page = post_login.follow(status=302)
        ok_('authtkt' in initial_page.request.cookies,
            "Session cookie wasn't defined: %s" % initial_page.request.cookies)
        ok_(initial_page.location.startswith('http://localhost/contribution/'),
    ...
    def test_logout(self):
        ...
        resp = self.app.get('/login_handler?login=%s&password=%s' % (self.test_user, self.test_pass),
                            status=302)
        resp = resp.follow(status=302)
        ...
        ok_(resp.location.startswith('http://localhost/authentication/post_logout'))
